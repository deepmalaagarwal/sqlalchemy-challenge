...
def precip():
    ...
    results = query = session.query(Measurement.date, Measurement.prcp).filter(Measurement.date >= last_year_date).all()
...
def temperature_tobs():
    ...
    results = session.query(Station.name, Measurement.date, Measurement.tobs).filter(Measurement.date >= "2017-01-01", Measurement.date <= "2018-01-01").all()
